...
You need to build a new 1-D array of size A by taking one element from each row of the 2-D matrix in such a way that
the cost of the newly built array is minimized.
The cost of an array is the minimum possible value of the absolute difference between any two adjacent elements of the array.
So if the newly built array is X, the element picked from row 1 will become X[1], element picked from row 2 will become
X[2],and so on.
...
Explanation 2:
 Newly build array : [3, 2, 4]. The minimum cost will be 1 since the minimum absolute difference will be
 1(3 - 2).
'''
...
def minDiff(A: int, B: int, C: List[List[int]])-> int:

    min_coast = float('inf')
    for col in range(B):
        cur_coast = 0
        for row in range(1, A):
            cur_coast += abs(C[row][col] - C[row-1][col])
        min_coast = min(min_coast, cur_coast)
    return min_coast
