...
SCALER organizes a series of contests aimed at helping learners enhance their coding skills. Each learner can participate
in multiple contests, and their participation is represented by integers in an array. The goal is to identify how frequently
each learner has participated in these contests. This information will help SCALER determine which learners are participating
...
def frequencyQuery(A: list, B: list)->list:
    ans = []
    for learner in B:
        count = 0
        for participant in A:
            if learner == participant:
                count+=1
        ans.append(count)
    return ans

# optimised approach.... using hashmap
def frequencyQuery_opt(A:list, B:list)->list:
    hashmap = {}
    ans = []
    for participant in A:
        hashmap[participant] = hashmap.get(participant, 0)+1

    for learner in B:
        ans.append(hashmap.get(learner, 0))
    return ans
...
if __name__ == "__main__":
    ...
    # A = [2, 5, 9, 2, 8]
    # B = [3, 2]
    print(frequencyQuery(A,B))
    print(frequencyQuery_opt(A,B))